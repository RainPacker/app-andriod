...
import com.tedu.chat.db.DemoDBManager;
import com.tedu.chat.db.InviteMessgeDao;
import com.tedu.chat.db.UserDao;
import com.tedu.chat.domain.EmojiconExampleGroupData;
import com.tedu.chat.domain.InviteMessage;
import com.tedu.chat.domain.RobotUser;
import com.tedu.chat.parse.UserProfileManager;
import com.tedu.chat.receiver.CallReceiver;
import com.tedu.chat.receiver.HeadsetReceiver;
import com.tedu.chat.ui.ChatActivity;
import com.tedu.chat.ui.VideoCallActivity;
import com.tedu.chat.ui.VoiceCallActivity;
...
//        options.setImPort(31097);
        ...
        boolean inited = EaseUI.getInstance().init(context, options);
        EMLog.d(TAG, "ease ui init result=" + inited);
        if (inited) {
            appContext = context;
...
//        options.setUseFCM(!HMSPushHelper.getInstance().isUseHMSPush());
    ...
    public class MyContactListener implements EMContactListener {
        ...
        public void onContactInvited(String username, String reason) {
            ...
            EventBus.getDefault().post(new IMEvent(IMEvent.IMEventTag.CONTACT_INVITED, username + "申请加你为好友，理由：" + reason));
        }
        ...
        public void onFriendRequestAccepted(String username) {
            ...
            EventBus.getDefault().post(new IMEvent(IMEvent.IMEventTag.CONTACT_AGREED, username + "接受了你的好友申请"));
        }
        ...
        public void onFriendRequestDeclined(String username) {
            ...
            EventBus.getDefault().post(new IMEvent(IMEvent.IMEventTag.CONTACT_INVITED, username + "拒绝了你的好友申请"));
        }
    }
        ...
        messageListener = new EMMessageListener() {
            ...
            public void onMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    if (Type.TXT.equals(message.getType())) {
                        JSONObject json = new JSONObject();
                        try {
                            json.putOpt("user", message.getFrom());
                        ...
                        if (body instanceof EMTextMessageBody) {
                            try {
                                json.putOpt("message", ((EMTextMessageBody) body).getMessage());
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                        }
                        IMEvent event = new IMEvent(IMEvent.IMEventTag.RECEIVER_TEXT_MESSAGE, json.toString());
                        EventBus.getDefault().post(event);
                        EMLog.w(TAG, "post event:" + event.getContent());
                    }
...
//                        getNotifier().onNewMsg(message);
    ...
    public void login(String user, String pwd, EMCallBack back) {
        EMClient.getInstance().login(user, pwd, back);
    }
